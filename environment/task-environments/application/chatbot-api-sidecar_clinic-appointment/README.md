...
Local HTTP chatbot product under test for `chat_clinic-appointment`.
Persona agent runtime: `application/shared-chat-persona`.

| Field | Value |
|---|---|
| Task | `chat_clinic-appointment` |
| `applicationId` | `clinic_booking_bot` |
| Compose service | `clinic-booking-chatbot` |
| Host port | 8908 |
| Upstream override env | `CHATBOT_UPSTREAM_CLINIC` |
...
```bash
docker compose up --build clinic-booking-chatbot
...
`chatbot-api-sidecar_acme-support-api`. Modeled on RasaHQ/rasa (https://github.com/RasaHQ/rasa); point `CHATBOT_UPSTREAM_CLINIC`
