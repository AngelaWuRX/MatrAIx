...
Local HTTP chatbot product under test for the four grounded `chat_*` persona
tasks. Persona agent runtime: `application/shared-chat-persona`.

One image (`./grounded-chatbot`) serves all four bots; the active persona is
...
| `chat_dev-helper` | `dev_helper_bot` | `dev-help-chatbot` | 8909 | `CHATBOT_UPSTREAM_DEV` |
...
Rogendo/Mental-health-Chatbot); an operator can swap in one of those by
pointing the task's `CHATBOT_UPSTREAM_*` env at its endpoint.
