"""Grounded chatbot sidecar (rule-based smoke implementation) for the five
`chat_*` persona tasks.
...
bot, a retail-support bot, a clinic booker, a budget coach). The active
persona is selected by the ``SIDECAR_BOT`` env var, so all five task services
build the same image and share this compose file, each pinned to one bot:
    ...
    budget_coach_bot   -> $3,200 income / $1,400 rent -> $300/mo savings plan
...
_BUDGET_ITEMS = [
    "Rent: $1,400",
    "Groceries: $400 (cook 5 nights/week)",
    "Dining out: $200 (down from ~$450 — your biggest leak)",
    "Subscriptions: $40 (cancel 2–3 unused)",
    "Transport + utilities: $360",
    "Everyday/flex: $500",
    "Savings (auto-transfer on payday): $300",
]


def _budget_coach(state: dict, msg: str) -> tuple[str, list[str]]:
    text = msg.lower()
    if _has(text, "eat out", "dining", "restaurant", "subscription", "3200",
            "3,200", "1400", "1,400", "income", "rent", "takeout"):
        state["facts_known"] = True
    if state.get("facts_known") or state["turn"] >= 1:
        state["resolved"] = True
        return (
            "Great — with **$3,200 take-home** and **$1,400 rent**, here's a "
            "split that banks your **$300** first:\n"
            "• Rent $1,400  • Groceries $400  • Dining out $200 "
            "(down from ~$450)  • Subscriptions $40  • Transport+utilities "
            "$360  • Flex $500  • **Savings $300**.\n"
            "The single biggest place to cut is **eating out** — dropping it "
            "to $200 and trimming 2–3 subscriptions frees up your whole "
            "savings target. Set the $300 as an **automatic transfer on payday** "
            "so it's gone before you can spend it. Want me to tweak any category?",
            _BUDGET_ITEMS,
        )
    return (
        "Let's build you a plan. You take home about $3,200 and pay $1,400 rent "
        "— roughly how much goes to eating out and subscriptions each month?",
        [],
    )
...
_HANDLERS: dict[str, Callable[[dict, str], tuple[str, list[str]]]] = {
    ...
    "budget_coach_bot": _budget_coach,
